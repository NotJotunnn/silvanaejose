...
const PaymentPopup = ({ props }: { props: PaymentPopupProps }) => {
  ...
  return (
    <div className={`PurchasePopup fixed top-0 left-0 w-screen h-screen flex flex-col justify-end md:justify-center items-center z-20 ${!showPopup ? "pointer-events-none" : ""}`}>
      <div className={`background bg-black/40 cursor-pointer w-full h-full absolute top-0 left-0 transition-all duration-700 ease-in-out ${showPopup ? "opacity-100" : "delay-700 opacity-0"}`} onClick={() => closePopup()}></div>
      <div className={`popup z-10 md:w-101 w-full md:h-101 h-[50dvh] bg-white rounded-t-4xl md:rounded-4xl p-9 flex flex-col justify-center items-center transition-all duration-700 ease-in-out ${showPopup ? "delay-700 translate-y-0" : "translate-y-[100dvh]"}`}>
